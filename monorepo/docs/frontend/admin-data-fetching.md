...
- **API endpoint format:** The base URL (`UNIXSEE_CORE_API_BASE_URL`) is always `.../api/v1`. When calling `serverFetch`, `clientFetch`, or `publicFetch`, pass the path **after** `/api/v1`. **Never** include `/v1/` in the endpoint — this creates a double `/api/v1/v1/...` URL. Use `/admin/tickets`, `/admin/users`, etc. — not `/v1/admin/tickets`.
  - ✅ `serverFetch('/admin/tickets')` → `http://host:4000/api/v1/admin/tickets`
  - ❌ `serverFetch('/v1/admin/tickets')` → `http://host:4000/api/v1/v1/admin/tickets`
