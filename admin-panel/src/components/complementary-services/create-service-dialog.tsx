...
import { activateComplementaryRequestAction } from "@/actions/complementary-services/complementary-service-actions";
...
function CreateServiceForm({
  ...
  const effectiveWebsiteId =
    mode === "request" ? request!.websiteId : websiteId;
  const effectiveFamily =
    mode === "request"
      ? request!.family
      : (family as ComplementaryServiceFamilyType);
  ...
  function validate() {
    ...
    if (!agreedAmount.trim())
      nextErrors.agreedAmount = "مبلغ توافق‌شده الزامی است.";
  ...
  if (step === "confirm") {
    return (
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        ...
        <DialogFooter>
          <Button type="button" onClick={submitConfirmed} disabled={submitting}>
            {submitting
              ? mode === "request"
                ? "در حال فعالسازی..."
                : "در حال ایجاد..."
        ...
        <div className="box-border w-full max-w-full space-y-5 px-4 pb-6">
          {mode === "request" && request ? (
            ...
            <div className="grid w-full min-w-0 grid-cols-1 gap-4">
              ...
              <div className="min-w-0 space-y-2">
                <label htmlFor="create-website" className="text-sm font-medium">
                  وب‌سایت *
                </label>
                <Select
                  value={websiteId}
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between gap-3 text-xs text-muted-foreground">
              ...
              >
                {errors.description ||
                  "حداقل ۲۰ نویسه؛ هدف، وضعیت فعلی و محدودیت‌ها"}
              </span>
